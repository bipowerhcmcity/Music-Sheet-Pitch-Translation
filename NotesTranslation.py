...
notes_height_BASS_1 = ['G3', 'A3', 'B3', 'C4', 'D4', 'E4', 'F4', 'G4','A4', 'B4']
...
#	MAIN_staff, BASS_staff = SeparateMainNSUB(staffs)
#	RE_ORDERED = []
#	for i in range(len(MAIN_staff)):
#		temp = [(0, "NONE", (0,0))]*len(MAIN_staff[i])
#		for j in BASS_staff[i]:
#			#print(j)
#			index = MinIndex(j[2], MAIN_staff[i])
#			temp[index] = j
#		RE_ORDERED.append(temp)
#	new_staffs = []
#	for i,j in zip(MAIN_staff, RE_ORDERED):
#	    new_staffs.append(i)
#	    new_staffs.append(j)
#	return new_staffs
	...
	if key == "MAIN":
		...
		for staff in data:
			...
			for note in staff:
				if note[3] == True:
					print(note[2], end = "  ")
					note = tuple([note[0], note[1], note[2] - 1,note[3],note[4]])					
					print(note[2])
				if note[2] <0 and abs(note[2]) < len(notes_height_1):
					...
					temp.append(tuple([note[0], note[1], "NONE", False, False]))
			result.append(temp)
		return result
	elif key == "BASS":
		result = []
